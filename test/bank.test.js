...
describe("Final Bank App Test Cases", () => {
    ...
    describe("Account Creation", () => {
        ...
        test("TP-001 - Create Valid Account", () => {
            const acc = bank.createAccount("Rohan", 100);
        ...
        test("TP-002 - Create Account with Empty Name", () => {
            expect(() => bank.createAccount("", 100)).toThrow();
        });

        test("TP-003 - Create Account with Negative Balance", () => {
            expect(() => bank.createAccount("Rohan", -10)).toThrow();
        });

        test("TP-004 - Unique Account IDs", () => {
            const acc1 = bank.createAccount("A", 100);
            const acc2 = bank.createAccount("B", 100);
    ...
    describe("Deposit", () => {
        ...
        test("TP-005 - Deposit Valid Amount", () => {
            const acc = bank.createAccount("Rohan", 100);
            const newBalance = bank.deposit(acc.id, 50);
        ...
        test("TP-007 - Deposit Negative Amount", () => {
            const acc = bank.createAccount("Rohan", 100);
            expect(() => bank.deposit(acc.id, -20)).toThrow();
        });

        test("TP-008 - Deposit Very Large Amount", () => {
            const acc = bank.createAccount("Rohan", 100);
            const newBalance = bank.deposit(acc.id, 1e9);
            expect(newBalance).toBe(100 + 1e9);
        });

        test("TP-009 - Deposit to Non-Existing Account", () => {
            expect(() => bank.deposit("FAKE-ID", 100)).toThrow();
        });
    });
    ...
    describe("Withdrawal", () => {
        ...
        test("TP-010 - Withdraw Valid Amount", () => {
            const acc = bank.createAccount("Rohan", 100);
            const newBalance = bank.withdraw(acc.id, 50);
            expect(newBalance).toBe(50);
        });

        test("TP-011 - Withdraw More Than Balance", () => {
            const acc = bank.createAccount("Rohan", 100);
            expect(() => bank.withdraw(acc.id, 200)).toThrow();
        });
        ...
        test("TP-013 - Withdraw Negative Amount", () => {
            const acc = bank.createAccount("Rohan", 100);
            expect(() => bank.withdraw(acc.id, -10)).toThrow();
        });

        test("TP-014 - Withdraw from Non-Existing Account", () => {
            expect(() => bank.withdraw("FAKE-ID", 50)).toThrow();
        });
    });
    ...
    describe("Balance Check", () => {
        ...
        test("TP-015 - Check Valid Account Balance", () => {
            const acc = bank.createAccount("Rohan", 300);
            expect(bank.getBalance(acc.id)).toBe(300);
        });

        test("TP-016 - Check Invalid Account Balance", () => {
            expect(() => bank.getBalance("FAKE-ID")).toThrow();
        });
    });
